...
void make_spot_checks(WordVector& v, WordMap& m) throw(string)
{
    ...
    for (string text : texts)
    {
        ...
        if(m.get_data()[text].get_count()>0)
        {
          vcount = vword.get_count();
          mcount = mword.get_count();
        }
...
void check_concordances(WordVector& v, WordMap& m) throw(string)
{
    ...
    while (itv != vdata.end())
    {
        ...
        if ((*itv).gettext() != (itm->second).gettext() || (*itv).get_count() != (itm->second).get_count())
        {
            throw string(string("Data mismatch: ")
                    + itv->gettext() + ":" + to_string(itv->get_count())
                    + " and " + itm->second.gettext() + ":"
...
void time_word_searches(WordVector& v, WordMap& m) throw(string)
{
    ...
    for (int i = 1; i <= MAX_SEARCHES; i++)
    {
        ...
        int index = rand()%size;
        string text = vdata[index].gettext();
